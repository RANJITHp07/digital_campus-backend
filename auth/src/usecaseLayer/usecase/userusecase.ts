...
export class Userusecase {
  ...
  async createUser({
    ...
    try {
      ...
      if (!user) {
        ...
        await this.publisher.publish("authExchange", "createroute", {
          id: createnewUser.id,
          username,
          email,
  ...
  async loginUser({ email, password }: { email: string; password: string }) {
    try {
      ...
      if (user && user.id) {
        ...
        const match: boolean = await this.bcrypt.compare(
          password,
          user.password
        );
        if (match) {
  ...
  async updateUser({ id, update }: { id: number; update: Partial<IUser> }) {
    try {
      ...
      if (update.profile && updatedUser) {
        await this.publisher.publish("authExchange", "updateProfile", {
          id: id,
  ...
  async verifyEmail({ email, username }: { email: string; username: string }) {
    try {
      ...
      const verify = await this.nodemailer.sendEmailVerification(
        email,
        username
      );
