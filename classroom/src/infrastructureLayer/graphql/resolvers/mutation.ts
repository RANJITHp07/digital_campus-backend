...
export const classroomMutation={
    ...
    async updateClass(_:unknown,args:{id:string,update:Partial<IClassroom>},context:any){
       try{
          const user=authenticate(context)
          if(user){
     ...
     async addStudent(_:unknown,args:{addstudent:{code:string,userId:string}},context:any){
        try{
         const user=authenticate(context)
          if(user){
     ...
     async deleteStudent(_:unknown,args:{deletedstudent:{code:string,userId:string}},context:any){
        try{
         const user=authenticate(context)
         if(user){
    ...
    async  addToAdmin(_:unknown,args:{id:string,classroomId:string},context:any){
        try{
         const user=authenticate(context)
          if(user){
     ...
     async  removeFromAdmin(_:unknown,args:{id:string,classroomId:string},context:any){
        try{
         const user=authenticate(context)
          if(user){
     ...
     async emailInvitation(_:unknown,args:{invitation:{fromEmail:string,toEmail: string, username: string,creator:string,code:string}},context:any){
           try{
            const user=authenticate(context)
          if(user){
